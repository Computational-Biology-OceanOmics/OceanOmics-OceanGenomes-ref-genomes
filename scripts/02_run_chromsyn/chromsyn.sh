...
# Define paths
RUNDIR=$(pwd)
LPATH=/scratch/pawsey0964/$USER/busco_db
SLIMSUITE=/software/projects/pawsey0964/singularity/SLiMSuite
...
for GENOME in ../*.fa; do
  ...
  if [ ! -f "$PREFIX.full.ctg.txt" ]; then
    ...
    singularity run "/software/projects/pawsey0964/singularity/barrnap:0.9.sif" barrnap --kingdom euk -o "$PREFIX.rrna.fa" --threads 16 < "$PREFIX.tmp.fasta" | tee "$PREFIX.rrna.gff"
...
for GENOME in ../*.fa; do
  ...
  if [ ! -f "$GENBASE.tidk.tsv" ]; then
    singularity run "/software/projects/pawsey0964/singularity/depthsizer:v1.9.0.sif" python "$SLIMSUITE/tools/telociraptor.py" tweak=F seqin="$GENOME" basefile="$GENBASE" i=-1 backups=F telonull=T
    singularity run "/software/projects/pawsey0964/singularity/depthsizer:v1.9.0.sif" python "$SLIMSUITE/tools/telociraptor.py" tweak=T seqin="$GENOME" basefile="$GENBASE" i=-1 backups=F log="$GENBASE.tweak"
    singularity run "/software/projects/pawsey0964/singularity/tidk:0.2.31.sif" tidk search -o "$GENBASE" -s AACCCT -d ./ -e tsv "$GENOME"
...
singularity run /software/projects/pawsey0964/singularity/depthsizer:v1.9.0.sif Rscript $SLIMSUITE/libraries/r/chromsyn.R $SETTINGS basefile=$sample.hapsyn | tee $sample.hapsyn.log
