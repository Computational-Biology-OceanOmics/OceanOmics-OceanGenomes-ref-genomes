#!/bin/bash
# Compile gfastats for hifi-only assemblies (p_ctg and a_ctg).
# Saves to hifi_gfastats_report.txt
# Usage: bash 04b_hifi_gfastats_compile.sh

base_dir="/scratch/pawsey0964/$USER/ref-gen/OG*"

echo -e "filename\tnum_contigs\tcontig_n50\tcontig_n50_size_mb\tnum_scaffolds\tscaffold_n50\tscaffold_n50_size_mb\tlargest_scaffold\tlargest_scaffold_size_mb\ttotal_scaffold_length\ttotal_scaffold_length_size_mb\tgc_content_percent" > hifi_gfastats_report.txt

tsv_files=$(find $base_dir -path "*/gfastats-hifi/*" \( -name "*p_ctg.assembly_summary.txt" -o -name "*a_ctg.assembly_summary.txt" \))
...
for file in $tsv_files; do
    awk -v filename="$(basename "$file")" 'BEGIN { OFS="\t" }
        ...
        }' "$file" >> hifi_gfastats_report.txt
done

echo "Written: hifi_gfastats_report.txt"
column -t hifi_gfastats_report.txt
