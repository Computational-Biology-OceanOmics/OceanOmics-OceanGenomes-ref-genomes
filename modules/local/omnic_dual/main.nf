process OMNIC {
    ...
    && pairtools parse --min-mapq 00 --walks-policy 5unique --max-inter-align-gap 30 --nproc-in 32 --nproc-out 32 --chroms-path "${meta.id}.${haplotype}.genome" "${meta.id}.${haplotype}.aligned.sam" >  "${meta.id}.${haplotype}.parsed.pairsam" \
